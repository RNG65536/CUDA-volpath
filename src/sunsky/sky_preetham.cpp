...
float3 expf(const float3& v) { return make_float3(expf(v.x), expf(v.y), expf(v.z)); }

float PreethamSunSky::rad2deg(float rads) { return rads * 180.0f / static_cast<float>(M_PI); }

float3 PreethamSunSky::Yxy2XYZ(const float3& Yxy)
{
    return make_float3(Yxy.y * (Yxy.x / Yxy.z), Yxy.x, (1.0f - Yxy.y - Yxy.z) * (Yxy.x / Yxy.z));
}

float3 PreethamSunSky::XYZ2rgb(const float3& xyz)
{
...
const PreethamSunSky::Datum PreethamSunSky::data[] = {
    {0.38f, 1655.9f, -1.f, -1.f},      {0.39f, 1623.37f, -1.f, -1.f},
    {0.4f, 2112.75f, -1.f, -1.f},      {0.41f, 2588.82f, -1.f, -1.f},
    {0.42f, 2582.91f, -1.f, -1.f},     {0.43f, 2423.23f, -1.f, -1.f},
    {0.44f, 2676.05f, -1.f, -1.f},     {0.45f, 2965.83f, 0.003f, -1.f},
    {0.46f, 3054.54f, 0.006f, -1.f},   {0.47f, 3005.75f, 0.009f, -1.f},

    {0.48f, 3066.37f, 0.014f, -1.f},   {0.49f, 2883.04f, 0.021f, -1.f},
    {0.5f, 2871.21f, 0.03f, -1.f},     {0.51f, 2782.5f, 0.04f, -1.f},
    {0.52f, 2710.06f, 0.048f, -1.f},   {0.53f, 2723.36f, 0.063f, -1.f},
    {0.54f, 2636.13f, 0.075f, -1.f},   {0.55f, 2550.38f, 0.085f, -1.f},
    {0.56f, 2506.02f, 0.103f, -1.f},   {0.57f, 2531.16f, 0.12f, -1.f},

    {0.58f, 2535.59f, 0.12f, -1.f},    {0.59f, 2513.42f, 0.115f, -1.f},
    {0.6f, 2463.15f, 0.125f, -1.f},    {0.61f, 2417.32f, 0.12f, -1.f},
    {0.62f, 2368.53f, 0.105f, -1.f},   {0.63f, 2321.21f, 0.09f, -1.f},
    {0.64f, 2282.77f, 0.079f, -1.f},   {0.65f, 2233.98f, 0.067f, -1.f},
    {0.66f, 2197.02f, 0.057f, -1.f},   {0.67f, 2152.67f, 0.048f, -1.f},

    {0.68f, 2109.79f, 0.036f, -1.f},   {0.69f, 2072.83f, 0.028f, 0.028f},
    {0.7f, 2024.04f, 0.023f, 0.023f},  {0.71f, 1987.08f, 0.018f, 0.018f},
    {0.72f, 1942.72f, 0.014f, 0.014f}, {0.73f, 1907.24f, 0.011f, 0.011f},
    {0.74f, 1862.89f, 0.01f, 0.01f},   {0.75f, 1825.92f, 0.009f, 0.009f}};
...
void PreethamSunSky::preprocess()
{
    ...
    const float xi =
        (4.0f / 9.0f - m_turbidity / 120.0f) * (static_cast<float>(M_PI) - 2.0f * m_sun_theta);

    float3 zenith;
    // Preetham paper is in kilocandellas -- we want candellas
    zenith.x =
        ((4.0453f * m_turbidity - 4.9710f) * tan(xi) - 0.2155f * m_turbidity + 2.4192f) * 1000.0f;
    zenith.y =
        m_turbidity * m_turbidity *
            (0.00166f * sun_theta_3 - 0.00375f * sun_theta_2 + 0.00209f * m_sun_theta) +
        m_turbidity *
            (-0.02903f * sun_theta_3 + 0.06377f * sun_theta_2 - 0.03202f * m_sun_theta + 0.00394f) +
        (0.11693f * sun_theta_3 - 0.21196f * sun_theta_2 + 0.06052f * m_sun_theta + 0.25886f);
    zenith.z =
        m_turbidity * m_turbidity *
            (0.00275f * sun_theta_3 - 0.00610f * sun_theta_2 + 0.00317f * m_sun_theta) +
        m_turbidity *
            (-0.04214f * sun_theta_3 + 0.08970f * sun_theta_2 - 0.04153f * m_sun_theta + 0.00516f) +
        (0.15346f * sun_theta_3 - 0.26756f * sun_theta_2 + 0.06670f * m_sun_theta + 0.26688f);

    float cos_sun_theta = cosf(m_sun_theta);

    float3 divisor_Yxy = (1.0f + m_c0 * expf(m_c1)) * (1.0f + m_c2 * expf(m_c3 * m_sun_theta) +
                                                       m_c4 * cos_sun_theta * cos_sun_theta);
    ...
    m_sun_dir           = make_float3(
        cosf(m_sun_phi) * sin_sun_theta, sinf(m_sun_phi) * sin_sun_theta, cosf(m_sun_theta));

    // perhaps transform from world space to local ONB space
    //     optix::Onb onb( m_up );
    //     onb.inverse_transform( m_sun_dir );

    // from local ONB to world space(assuming up=(0,1,0))
    m_sun_dir = make_float3(m_sun_dir.y, m_sun_dir.z, m_sun_dir.x);
}

// void PreethamSunSky::setVariables( Context& context )
// {
//     preprocess();
//
//     m_c0              = context.c0              ;
...
//     m_c3              = context.c3              ;
//     m_c4              = context.c4              ;
//     m_inv_divisor_Yxy = context.inv_divisor_Yxy ;
//     m_up              = context.sky_up          ;
//     m_overcast        = context.overcast        ;
//     m_sun_dir         = context.sun_direction   ;
//     m_sun_color       = context.sun_color       ;
// }

// lambda is wavelength in micro meters
float PreethamSunSky::calculateAbsorption(
    float sun_theta, float m, float lambda, float turbidity, float k_o, float k_wa)

{
    ...
    float water_vapor =
        k_wa > 0.0f ? expf(-0.2385f * k_wa * w * m / powf(1.0f + 20.07f * k_wa * w * m, 0.45f))
                    : 1.0f;
...
float3 PreethamSunSky::sunColor()
{
    ...
    for (int i = 0; i < 38; ++i)
    {
        ...
        results[i] *= calculateAbsorption(
            m_sun_theta, m, data[i].wavelength, m_turbidity, data[i].k_o, data[i].k_wa);
    }
...
float3 PreethamSunSky::skyColor(const float3& direction, bool CEL /* true for explicit light */)
{
    preprocess();
    ...
    if (m_overcast < 1.0f)
    {
        ...
        else
        {
            ...
            float3 color_Yxy =
                (make_float3(1.0f) + m_c0 * make_float3(expf(A.x), expf(A.y), expf(A.z))) *
                (make_float3(1.0f) + m_c2 * make_float3(expf(B.x), expf(B.y), expf(B.z)) +
                 m_c4 * gamma * gamma);
            color_Yxy = color_Yxy * m_inv_divisor_Yxy;

            // color_Yxy.y = 0.33f + 1.2f * ( color_Yxy.y - 0.33f ); // Pump up
            // chromaticity a bit color_Yxy.z = 0.33f + 1.2f * ( color_Yxy.z -
            // 0.33f ); //

            float3 color_XYZ = Yxy2XYZ(color_Yxy);
            sunlit_sky_color = XYZ2rgb(color_XYZ);
            sunlit_sky_color =
                sunlit_sky_color * 1.0e-3f;  // We are choosing to return kilo-candellas / meter^2
        }
    }
